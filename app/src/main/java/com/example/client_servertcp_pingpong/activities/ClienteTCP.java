...
public class ClienteTCP extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.v("PDM", "Variável: "+ jogadorClient.getCEPOponente());
    }
    ...
    public void conectarCodigo() {
        ...
        String result = "";
        try {
            clientSocket = new Socket(ip, 9090);//<---------------------------conecta Cliente por Socket
            tvStatus.post(new Runnable() {
                @Override
                public void run() {
            ...
            while (fromServer != null) {
                ...
                //jogadorClient.setCEP(CEPvalido);
                Log.v("PDM", "2. CEP Cliente: "+ jogadorClient.getCEP() + ", CEP Oponente: " + jogadorClient.getCEPOponente());
                result = fromServer.readUTF();
                if (result.substring(0,3).compareTo("CEP") == 0){
                    CEPOponente = result.substring(3);

                }
                if (CEPOponente != "" && CEPOponente != null) {
                ...
                if (jogadorClient.getCEP() != null && jogadorClient.getCEPOponente() != null){
                    if(jogadorClient.getCEP() != "" && jogadorClient.getCEPOponente() != "") {
            ...
            public void run() {
                try {
                    if (socketOutput != null){
                        ...
                        socketOutput.writeUTF("CEP"+jogadorClient.getCEP());
                        socketOutput.flush();
                    } else {
                        Log.v("PDM", "else");
